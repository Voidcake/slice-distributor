...
export function PizzaChart({ slicesByType, showLegend = false }: PizzaChartProps) {
  ...
  if (slices.length === 0) {
    return (
      <div className="w-full aspect-square rounded-full border-2 border-dashed border-gray-200 flex items-center justify-center">
    ...
    <div className="relative w-full aspect-square">
      <svg viewBox="0 0 100 100" className="w-full h-full">
        ...
        {slices.map((slice, index) => {
          ...
          return (
            <path key={index} d={pathData} fill={slice.color} stroke="#fff" strokeWidth="0.5" />
          );
        })}
      </svg>
    </div>
